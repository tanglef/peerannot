...
class LIWMV(CrowdModel):
    """
    ========================================
    Iterative Weighted Majority Vote
    ========================================

    Label dependent Iterative Weighted Majority Vote (LIWMV) is a model that iteratively updates the weight of each worker based on their performance. The weight of each worker is updated at each iteration based on their accuracy in the previous iteration of weighted majority vote aggregation  for each class.
    """
    ...
    def __init__(self, answers, n_classes=2, sparse=False, **kwargs):
        ...
        .. math::
            ...
            \\mathrm{LIWMV}(i, \mathcal{D})^t = \\underset{k\\in[K]}{\\mathrm{argmax}} \\sum_{j\\in\\mathcal{A}(x_i)}\\beta_j^{t,k}\\mathbf{1}(y_i^{(j)} = k)

        with

        .. math::

            \\beta_j^{t,k} = \\frac{1}{|y|y_i^{(j)}=y|} \sum_{i'=1}^{n_{\\texttt{task}}} \mathbf{1}\\left(y_{i'}^{(j)} = \mathrm{LIWMV}(i', \{y_{i'}^{(j)}\}_j)^{t-1}\\right) \mathbf{1}(y_{i'}^{(j)}=k).
    ...
    def compute_baseline(self, weight=None):
        ...
        for task_id in list(self.answers.keys()):
            ...
            for worker, vote in task.items():
                baseline[task_id, vote] += weight[int(worker), vote]
    ...
    def accuracy_by_mv(self):
        """Compute accuracy of each worker based on previously aggregated labels for each class"""
        worker_score = np.zeros(self.n_workers, self.n_classes)
        worker_n_ans = np.zeros(self.n_workers, self.n_classes)
        for task_id, mv_lab in zip(self.answers.keys(), self.ans):
            for worker, lab in self.answers[task_id].items():
                if int(lab) == mv_lab:
                    worker_score[int(worker), int(lab)] += 1
                worker_n_ans[int(worker), int(lab)] += 1
        ...
        self.worker_score = worker_score / worker_score.sum(axis=0, keepdims=True)
    ...
    def run(self, nb_iter=10, **kwargs):
        ...
        self.MV(weight=1 / self.n_workers * np.ones(self.n_workers, self.n_classes))
        self.accuracy_by_mv()
